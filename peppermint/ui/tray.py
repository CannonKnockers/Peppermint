"""The Peppermint panel indicator.
...
The icon changes with the state of the work:
    ...
    idle       Peppermint waits for an idea
    working    Peppermint runs a task now
    attention  Peppermint needs your approval or your answer
...
log = logging.getLogger("peppermint.tray")
...
def pick_icon(tasks: list[dict]) -> tuple[str, str]:
    ...
    user must act before Peppermint can continue.
    """
    if not tasks:
        return ICON_IDLE, "Peppermint — ready for an idea"
...
class Tray:
    ...
    def _build_icon(self) -> None:
        if HAVE_XAPP:
            ...
            self.icon.set_name("peppermint")
            self.icon.set_icon_name(ICON_IDLE)
            self.icon.set_tooltip_text("Peppermint")
        ...
        self.fallback.set_tooltip_text("Peppermint")
    ...
    def _menu(self) -> Gtk.Menu:
        ...
        open_item = Gtk.MenuItem(label="Open Peppermint")
        ...
        quit_item = Gtk.MenuItem(label="Hide the Peppermint icon")
    ...
    def _health_line(self) -> str:
        try:
            ...
            return "The Peppermint daemon is not running"
        if not data.get("ok"):

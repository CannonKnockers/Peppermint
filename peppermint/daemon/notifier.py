...
log = logging.getLogger("peppermint.notifier")
...
def send(title: str, body: str, urgent: bool = False, action_label: str = "") -> None:
    """Show a notification. A click on the action opens the Peppermint window."""
...
def _on_open(notification, action, user_data):
    """Open the Peppermint window when the user clicks the notification."""
    try:
        from peppermint.common import dbus_api

        dbus_api.call_window("Toggle")
    except Exception as exc:
        log.warning("Peppermint could not open the window: %s", exc)

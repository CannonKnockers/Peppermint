...
def _reject(reason: str) -> ToolError:
    return ToolError(f"Peppermint did not open that address, because {reason}.")
...
def normalise(raw: str) -> str:
    ...
    if match and not HOST_PORT.match(candidate):
        ...
        if scheme not in ALLOWED_SCHEMES:
            raise _reject(
                f"`{scheme}:` is not a web address. Peppermint opens only http and https"
            )
    else:
...
def open_url(url: str):
    ...
    if not opener:
        raise ToolError(
            "The `xdg-open` command is missing, so Peppermint cannot reach your browser."
        )
    ...
    if proc.returncode != 0:
        ...
        raise ToolError(f"Peppermint could not open {address}: {detail}")

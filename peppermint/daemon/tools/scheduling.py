"""Scheduling tools. Peppermint uses the user crontab."""
...
from peppermint.daemon.tools.registry import Confirm, Context, ToolError, tool
...
def _read_crontab() -> str:
    ...
    if proc.returncode != 0 and "no crontab" not in proc.stderr.lower():
        raise ToolError(f"Peppermint could not read the crontab: {proc.stderr.strip()}")
    return proc.stdout
...
def _write_crontab(text: str) -> None:
    ...
    if proc.returncode != 0:
        raise ToolError(f"Peppermint could not write the crontab: {proc.stderr.strip()}")

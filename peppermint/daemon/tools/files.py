...
def read_file(path: str, max_lines: int = 0):
    ...
    if not verdict.safe:
        raise ToolError(f"Peppermint does not read this path, because it {verdict.reason}.")
    if not target.exists():
        raise ToolError(f"`{target}` does not exist.")
    if target.is_dir():
    ...
    try:
        ...
        raise ToolError(f"Peppermint could not read `{target}`: {exc}")
...
def write_file(path: str, content: str, executable: bool = False, ctx: Context = None):
    ...
    try:
        ...
        raise ToolError(f"Peppermint could not write `{target}`: {exc}")
    if executable:
...
@tool(
    name="make_dir",
    description="Make a folder. Peppermint makes the parent folders too. Use this before you move files into a new folder.",
    parameters={
        "type": "object",
...
def make_dir(path: str, ctx: Context = None):
    ...
    try:
        ...
        raise ToolError(f"Peppermint could not make `{target}`: {exc}")
...
@tool(
    ...
    description=(
        ...
        "Peppermint puts the file inside that folder and makes the folder if it is missing. "
...
def move_file(src: str, dst: str, ctx: Context = None):
    ...
    try:
        ...
        raise ToolError(f"Peppermint could not move the file: {exc}")
    if ctx and ctx.db:
...
@tool(
    ...
    description=(
        ...
        "Peppermint makes each folder, moves every matching file, and reports what it did. "
...
@tool(
    name="delete_file",
    description="Send a file or folder to the trash. This always needs approval. Peppermint never erases data permanently.",
    parameters={
        "type": "object",
        "properties": {"path": {"type": "string"}},
        "required": ["path"],
...
def delete_file(path: str, ctx: Context = None):
    ...
    raise ToolError("The `gio` command is missing, so Peppermint cannot use the trash safely.")

...
class Daemon:
    ...
    def start(self) -> int:
        if not self.db.integrity_ok():
            log.error("The Peppermint database fails its integrity check. Peppermint stops "
        ...
        for task_id in recovered["halted"]:
            ...
            log.warning("Task %s stopped part way through a change. Peppermint did not "
                        "repeat it.", task_id)
            notifier.send(
                "Peppermint stopped part way through a change",
        ...
        Gio.bus_own_name(
            ...
            lambda *_: log.info("Peppermint owns %s", dbus_api.DAEMON_NAME),
            self._on_name_lost,
        )
        ...
        log.info("Peppermint is ready.")
        self.loop.run()
        return 0

    def _on_signal(self) -> bool:
        log.info("Peppermint stops.")
        self.jobs.put(None)
        self.loop.quit()
    ...
    def _on_name_lost(self, connection, name):
        if connection is None:
            log.error("Peppermint cannot reach the session bus.")
        else:
            log.error("Another Peppermint daemon already runs.")
        self.loop.quit()
        ...
        while True:
            ...
            try:
                ...
                self.db.set_status(job.task_id, Status.FAILED,
                                   error="Peppermint hit an internal fault. See the log.")
    ...
    def _announce(self, task_id: int, result) -> None:
        ...
        if result.status is Status.DONE:
            notifier.send("Peppermint finished", f"{idea}\n{result.text[:180]}", action_label="Open")
        elif result.status is Status.AWAITING_CONFIRMATION:
            notifier.send("Peppermint needs your approval", result.text[:180], urgent=True, action_label="Open")
        elif result.status is Status.AWAITING_INPUT:
            notifier.send("Peppermint has a question", result.text[:180], urgent=True, action_label="Open")
        elif result.status is Status.FAILED:
            notifier.send("Peppermint could not finish", f"{idea}\n{result.text[:180]}", action_label="Open")

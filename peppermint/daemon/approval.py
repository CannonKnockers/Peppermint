...
def verify(confirmation: dict, tool: str, args: dict, task_id: int) -> None:
    ...
    if not stored:
        raise ApprovalError(
            "This approval carries no token, so Peppermint cannot prove it was for "
            "this action. Peppermint did nothing."
        )
    if stored != expected:
        raise ApprovalError(
            "The action changed after you approved it. Peppermint did nothing. "
    ...
    if is_expired(confirmation.get("expires_at", "")):
        raise ApprovalError(
            ...
            "so Peppermint did not use it. The machine may have changed since it asked."
        )

    changed = compare_targets(confirmation.get("fingerprint", ""), target_fingerprint(args))
    if changed:
        raise ApprovalError(
            f"The target changed after you approved it: {changed}. Peppermint did nothing."
        )

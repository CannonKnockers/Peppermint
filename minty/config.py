"""All tunable parameters for Minty live here.

Environment variables with the MINTY_ prefix override the defaults, so you can
try a different model without editing code:

    MINTY_MODEL=qwen2.5:7b-instruct-q4_K_M minty-daemon
"""
...
def _env(name: str, default: str) -> str:
    return os.environ.get(f"MINTY_{name}", default)


def _env_int(name: str, default: int) -> int:
    try:
        return int(os.environ.get(f"MINTY_{name}", default))
    except ValueError:
        return default
...
def _env_float(name: str, default: float) -> float:
    try:
        return float(os.environ.get(f"MINTY_{name}", default))
    except ValueError:
        return default
...
# An approval is an answer about the machine as it was when Minty asked.
# After this many seconds the question is stale and Minty asks again.
APPROVAL_TTL_S = _env_int("APPROVAL_TTL_S", 900)

# --- Storage ---------------------------------------------------------------

DATA_DIR = Path(_env("DATA_DIR", str(HOME / ".local/share/minty")))
DB_PATH = DATA_DIR / "minty.db"
LOG_PATH = DATA_DIR / "minty.log"

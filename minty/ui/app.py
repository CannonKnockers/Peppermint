"""The Minty window application.

The application owns the bus name `org.minty.Window`. The hotkey command
`minty toggle` calls `Toggle` on that name. Only one window can exist.
"""
...
from minty.ui.tray import Tray  # noqa: E402
from minty.ui.window import MintyWindow  # noqa: E402

log = logging.getLogger("minty.ui")


class MintyApp(Gtk.Application):
    def __init__(self, background: bool = False):
        super().__init__(application_id="org.minty.App",
                         flags=Gio.ApplicationFlags.FLAGS_NONE)
        self.window: MintyWindow | None = None
    ...
    def do_startup(self) -> None:
        ...
        Gio.bus_own_name(
            ...
            None,
            lambda *_: log.warning("Another Minty window already runs."),
        )
        try:
            self.tray = Tray(self)
        except Exception:
            log.exception("Minty could not make the panel icon.")

    def do_activate(self) -> None:
        # The panel icon keeps Minty alive when no window is open.
        self.hold()
    ...
    def _ensure_window(self) -> MintyWindow:
        if self.window is None:
            self.window = MintyWindow(self)
        return self.window
...
def main() -> int:
    ...
    return MintyApp(background=background).run(argv)

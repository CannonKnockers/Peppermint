...
log = logging.getLogger("minty.llm")
    ...
    def ensure_model(self) -> str:
        ...
        for name in names:
            if name.split(":")[0] == stem:
                log.warning("Model %s is missing. Minty uses %s.", self.model, name)
                self.model = name
                self._checked = True
                return self.model
        if config.FALLBACK_MODEL in names:
            log.warning("Model %s is missing. Minty uses the fallback %s.", self.model, config.FALLBACK_MODEL)

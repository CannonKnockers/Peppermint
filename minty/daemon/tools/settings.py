...
@tool(
    name="gsettings_set",
    description=(
        "Change one desktop setting. Minty applies a normal desktop setting at once and "
...
def list_themes(kind: str):
    ...
    if not names:
        return f"Minty found no {kind} theme."
...
@tool(
    name="set_keybinding",
    description=(
        "Make a custom keyboard shortcut in Cinnamon. Minty puts it in the first free slot. "

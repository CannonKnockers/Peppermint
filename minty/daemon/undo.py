...
def _revert_gsettings(record: dict) -> Result:
    ...
    if len(target) != 2:
        return Result(False, f"Minty cannot read the setting name in `{record.get('target')}`.")
    schema, key = target
    old = record.get("old_value", "")
    if not old:
        return Result(False, f"Minty has no earlier value for {schema} {key}.")
...
def _revert_move(record: dict) -> Result:
    ...
    if not now_at.exists():
        return Result(False, f"`{now_at}` is not there any more, so Minty cannot move it back.")
    if was_at.exists():
        return Result(False, f"`{was_at}` exists again. Minty did not replace it.")
    try:
        was_at.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(now_at), str(was_at))
    except OSError as exc:
        return Result(False, f"Minty could not move the file back: {exc}")
...
def _revert_file(record: dict) -> Result:
    ...
    try:
        ...
        temporary = target.with_name(target.name + ".minty-undo")
        temporary.write_text(old)
        temporary.replace(target)
    except OSError as exc:
        return Result(False, f"Minty could not restore `{target}`: {exc}")
...
def revert(record: dict) -> Result:
    ...
    if reverter is None:
        return Result(False, f"Minty does not know how to undo a `{kind}` change.")
